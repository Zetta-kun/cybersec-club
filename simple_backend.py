...
# ==================== SADƏ HASH FUNKSİYASI ====================
def hash_password(password: str) -> str:
    """bcrypt əvəzinə hashlib SHA-256"""
...
async def login(login_data: UserLogin):
    ...
    access_token = jwt.encode(
        {"sub": login_data.username, "exp": datetime.utcnow() + timedelta(days=7)},
        SECRET_KEY,
        algorithm=ALGORITHM
    )
...
async def get_challenges(
    ...
    for cid, challenge in challenges_db.items():
        ...
        if difficulty and challenge.get("difficulty") != difficulty:
            continue
        if search and search.lower() not in challenge.get("title", "").lower():
            continue
        ...
        if current_user:
            solved = any(
                s["username"] == current_user["username"] and s["challenge_id"] == cid
...
async def get_challenge(challenge_id: str, current_user=Depends(get_current_user)):
    ...
    if not challenge:
        raise HTTPException(404, "Challenge tapilmadi")
    ch = challenge.copy()
    total = ch["total_attempts"] if ch["total_attempts"] > 0 else 1
    ch["solve_rate"] = round((ch["total_solves"] / total) * 100, 1)
    if current_user:
        solved = any(
            s["username"] == current_user["username"] and s["challenge_id"] == challenge_id
            for s in submissions_db
        )
        ch["is_solved"] = solved
    return ch
...
async def submit_flag(submit: FlagSubmit, current_user=Depends(get_current_user)):
    if not current_user:
    ...
    already = any(
        s["username"] == current_user["username"] and s["challenge_id"] == submit.challenge_id
        for s in submissions_db
    )
    if already:
        raise HTTPException(400, "Bu suali artiq hell etmisiniz")
    ...
    if check_flag(submit.challenge_id, submit.flag):
        ...
        current_user["rating_history"].append({
        ...
        if challenge["total_solves"] > 1:
            challenge["current_points"] = max(
                int(challenge["base_points"] * 0.3),
                challenge["current_points"] - 5
            )

        return {
            "status": "correct",
            "message": f"✅ Tebrikler! +{points} XP, +{rating_increase} Rating!",
            "points_earned": points,
            "is_first_blood": challenge["total_solves"] == 1,
            "attempts": 1
        }
    else:
        challenge["total_attempts"] += 1
        return {
            "status": "incorrect",
            "message": "❌ Yanlish flag! Yeniden cehd edin!",
            "attempts": 1
        }
    ...
    for s in submissions_db:
        if s["username"] == user["username"]:
            ...
            solved_challenges.append({
                "id": s["challenge_id"],
                "title": ch.get("title", ""),
                "points": ch.get("current_points", 0),
...
async def get_leaderboard():
    ...
    for idx, user in enumerate(users[:50], 1):
        r = user["rating"]
        if r >= 3000:
            rank_name, rank_color = "Legendary Grandmaster", "#AA0000"
        elif r >= 2400:
            rank_name, rank_color = "Grandmaster", "#FF7777"
        elif r >= 1900:
            rank_name, rank_color = "Candidate Master", "#FF88FF"
        elif r >= 1600:
            rank_name, rank_color = "Expert", "#AAAAFF"
        elif r >= 1400:
            rank_name, rank_color = "Specialist", "#77DDBB"
        elif r >= 1200:
            rank_name, rank_color = "Pupil", "#77FF77"
        else:
            rank_name, rank_color = "Newbie", "#CCCCCC"
...
async def get_wishlist():
    return {
        "wishlist": sorted(wishlist_db, key=lambda w: w["votes"], reverse=True),
        "total": len(wishlist_db)
    }
...
async def admin_dashboard(current_user=Depends(get_current_user)):
    if not is_admin(current_user):
        raise HTTPException(403, "Admin icazesi teleb olunur")
    ...
    return {
        ...
        "category_distribution": [
            {"category": cat, "total": data["total"], "solves": data["solves"]}
            for cat, data in cat_stats.items()
        ],
        "top_users": [
            {"username": u["username"], "rating": u["rating"], "solved": u["total_solved"]}
            for u in sorted(users_db.values(), key=lambda x: x["rating"], reverse=True)[:10]
        ],
        "recent_activities": []
    }
...
async def admin_create_challenge(challenge: ChallengeCreate, current_user=Depends(get_current_user)):
    ...
    challenges_db[cid] = {
        ...
        "current_points": challenge.base_points,
        "total_solves": 0,
        "total_attempts": 0,
        "tags": challenge.tags,
...
async def get_contests():
    contests = list(contests_db.values())
...
async def root():
    return {
        "name": "CyberSec Club API",
        "status": "operational",
        "version": "4.0.1",
        "features": ["Contests", "Wishlist", "Statistics", "Rating"]
    }
...
async def health():
    return {
        "status": "healthy",
        "users": len(users_db),
        "challenges": len(challenges_db),
        "contests": len(contests_db),
        "wishlist": len(wishlist_db)
    }